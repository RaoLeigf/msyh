...
namespace SUP.Common.Base
{
    ...
    public class I6WebAppInfo : NGWebAppInfo
    {
        ...
        private string _extendObj = string.Empty;//扩展对象,json串
        private bool isOAUser = true;//用户是否为OA用户
        ...
        public string DBVendor
        {
            get
            {
                ...
                string str = string.IsNullOrWhiteSpace(pubConnectString) ? UserConnectString.ToUpper() : pubConnectString;

                if (str.IndexOf("SQLCLIENT") > 0)
                {
                    return "SqlServer";
                }
                else if (str.IndexOf("ORACLECLIENT") > 0)
                {
                    return "Oracle";
                }
                return String.Empty;
        ...
        {
            get { return _extendObj; }
            set { _extendObj = value; }
        }
