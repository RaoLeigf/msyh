...
Ext.extend(Ext.App, Ext.util.Observable, {
    ...
    */
    addMessage: function (status, msg, delay) {
        ...
        if (!delay) {
            delay = 3;    // <-- default delay of msg box is 3 second.
    ...
    */
    ...
    handleResponse: function (res) {
        ...
        var msg = res.message || res.raw.message;
        if (msg.length > 0) {
            this.setAlert(msg, res.status, res.delay);
        }
    },

    handleException: function (res) {
        Ext.Msg.show({
            title: res.type ? res.type.toUpperCase() : '',
            msg: res.message || ($ajaxInfo && unescape($ajaxInfo.msg)),
            icon: Ext.Msg.ERROR,
