...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
  ...
  },
  {
    ...
    "mapping": "OName"
				...
				url: C_ROOT + 'GQT/QT/QTIndividualInfo/GetQTIndividualInfoNoOrg?id='+grid.getSelectionModel().getSelection()[0].data.PhId,
				autoLoad:false
			});
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
			...
			autoLoad: false,
			//url: C_ROOT + 'GQT/QT/CorrespondenceSettings/GetCorrespondenceSettingsListbyRelation'
			url: C_ROOT + 'GQT/QT/QTIndividualInfo/GetQTIndividualInfoOrg?id='+grid.getSelectionModel().getSelection()[0].data.PhId,
		}),
        ...
        me.buttons = ['->', {
            ...
            handler: function () {
                ...
                if(resultStore.getCount()>0){
                    for(var i=0;i<resultStore.getCount();i++){
                        codeStr=codeStr+resultStore.getAt(i).get('OCode')+',';
                        nameStr=nameStr+resultStore.getAt(i).get('OName')+',';
                    }
                    codeStr=codeStr.substr(0,codeStr.length-1);
                    nameStr=nameStr.substr(0,nameStr.length-1);
                }

                var dataindex=grid.getSelectionModel().getSelection()[0].index;
                grid.getStore().getAt(dataindex).set('DEFSTR9', codeStr);
                grid.getStore().getAt(dataindex).set('DEFSTR10', nameStr);
