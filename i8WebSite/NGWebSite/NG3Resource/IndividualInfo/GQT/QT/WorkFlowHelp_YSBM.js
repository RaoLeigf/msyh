...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
    ...
    initComponent: function () {
        var me = this;
		var code = grid.getSelectionModel().getSelection()[0].data.UserNo;
        me.initParam();
        var store = Ext.create('Ext.ng.JsonStore', {
			...
			url:C_ROOT + 'GQT/QT/CorrespondenceSettings/GetRelationYSBMLeftList'
        });
		store.load({
			params:{
				'userCode':code
			...
			callback:function(records,operation,success){
				Ext.getCmp('leftlabel').setText("共"+store.getCount()+"行");
			}
		});
		resultStore = Ext.create('Ext.ng.JsonStore', {
			...
			url:C_ROOT + 'GQT/QT/CorrespondenceSettings/GetRelationYSBMRightList'
		});
		resultStore.load({
			params:{
				'userCode':code
		...
		gridLeft = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
		            me.copyData([record], resultStore,store,'leftlabel','rightlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		gridRight = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {		            
		            me.copyData([record], store,resultStore,'leftlabel','rightlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		    ...
		    items: [{
		        ...
		        handler: Ext.bind(function () {
					...
					if(data.length < 1){
						...
						me.copyData(data, resultStore,store,'leftlabel','rightlabel');
					}
		        })
		    }, {
		        xtype: 'button',
		        ...
		        handler: Ext.bind(function () {
					...
					if(data.length < 1){
						...
						me.copyData(data, store,resultStore,'rightlabel','leftlabel');
					}
		        })
		    }, {
		        xtype: 'button',
		        ...
		        width: 60,
		        handler: Ext.bind(function () {
					var data = resultStore.data.items;
		            me.copyData(data, store,resultStore,'rightlabel','leftlabel');
		        })
		    }]
		};
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=store.find('OCode',Ext.getCmp('dingwei1').getValue());
						var mcindex=store.find('OName',Ext.getCmp('dingwei1').getValue());
						if(dmindex>=0){
        ...
        me.buttons = ['->', {
            ...
            handler: function () {
                /*if (me.callback) {
                    var data = me.currData, tmpdata = { "id": me.actNodeId, users: [] };
				...
				for(var i = 0;i<resultStore.getCount();i++){
					...
					var parentId= record.get('ParentOrgId');
					var ocode = record.get('OCode');
					var oname = record.get('OName');
					jsonArray.push({'PhId':phid,'OCode':ocode,'OName':oname,'ParentOrgId':parentId,'key':phid});
				}
				...
				myMask.show();
				Ext.Ajax.request({
						params:{'UserNo':code,'Models':myData},
						url:C_ROOT + 'GQT/QT/CorrespondenceSettings/RelationYSBMSetting',
						success:function(response){
							var resp = Ext.JSON.decode(response.responseText);
    ...
    copyData: function (selectData, resultStore,fromStore,fromlabel,resultlabel) {
        
		var index = resultStore.getCount();
        ...
        if(dataLen>1){
			for(var i=0;i<dataLen;i++){
				resultStore.insert(index+i, selectData[i]);
