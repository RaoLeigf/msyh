...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
  ...
  },
  {
    ...
    "mapping": "OName"
				...
				url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/GetBMListNoDYGXdtl?Dwdm='+gridGKXM.getSelectionModel().getSelection()[0].data.FProjCode+'&OrgId='+mainPanel_form.queryById("Org").getValue(),
				autoLoad:false
			});
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
			...
			autoLoad: false,
			//url: C_ROOT + 'GQT/QT/CorrespondenceSettings/GetCorrespondenceSettingsListbyRelation'
			url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/GetBMListDYGXdtl?Dwdm='+gridGKXM.getSelectionModel().getSelection()[0].data.FProjCode
		}),
		...
		bottom = {
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=resultStore.find('OCode',Ext.getCmp('dingwei2').getValue());
						var mcindex=resultStore.find('OName',Ext.getCmp('dingwei2').getValue());
						if(dmindex>=0){
		...
		storeZJ.load(function(){
			Ext.getCmp('leftlabel').setText("共"+storeZJ.getCount()+"行");
		});
		...
		resultStore.load(function(){
			...
			for(var i=0;i<resultStore.getCount();i++){
				...
				PhIdData.push(GXPhId);
										...
										/*var params={'Dylx':'96','Dwdm':dataPhid,'DefStr1':datacode};
										...
										resultStore.load(function(){
											...
											for(var i=0;i<resultStore.getCount();i++){
												...
												var zjindex=me.findStoreIndex(storeZJ,'DM',ZJLYDYDM);
												if(zjindex>=0)
												{
							...
							storeZJ.load(function(){
								for(var i=0;i<DMData.length;i++){
									...
									var zjindex=me.findStoreIndex(storeZJ,'DM',DMData[i]);
									if(zjindex>=0){
        ...
        me.buttons = ['->', {
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					var record=resultStore.getAt(i);
					var dataGKXM = gridGKXM.getSelectionModel().getSelection();
					var FProjCode=dataGKXM[0].data.FProjCode;
					//var	orgcode=dataGKXM[0].data.OCode;
					
					if(record.get('PhId')=="") {
						
						InsertData.push({'Dwdm':FProjCode,'Dylx':'GKBM','Dydm':record.get('OCode')});
				...
				Ext.Ajax.request({
						params:{'mydelete':PhIdData,'myinsert':InsertData},
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/UpdateGKXM',
						success: function (response) {
							var resp = Ext.JSON.decode(response.responseText);
