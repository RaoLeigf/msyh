...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
...
},
  {
    ...
    "mapping": "PhId"
		...
		var params={'Dylx':'03','Dwdm':dataPhid,'DefStr1':datacode};
										...
										resultStore.load(function(){
											...
											for(var i=0;i<resultStore.getCount();i++){
												...
												var YSKindex=me.findStoreIndex(storeYSK,'DM',YSKDM);
												if(YSKindex>=0)
												{
							...
							storeYSK.load(function(){
								for(var i=0;i<DMData.length;i++){
									...
									var YSKindex=me.findStoreIndex(storeYSK,'DM',DMData[i]);
									if(YSKindex>=0)
									{										
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					...
					if(record.get('PhId')==undefined) {
						...
						InsertData.push({'PhId':record.get('yskphid'),'DM':record.get('DM'),'key':record.get('yskphid')});
				...
				Ext.Ajax.request({
						...
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings/UpdateYSK',
						success: function (response) {
							var resp = Ext.JSON.decode(response.responseText);
							if (resp.Status === "success"){
								...
								var data = gridOrg.getSelectionModel().getSelection();
									var datacode=data[0].data.OCode;
									var dataPhid=data[0].data.PhId;
									var storeDYGX=gridDYGX.getStore();
									var storeYSK=gridYSK.getStore();
									var params={'Dylx':'03','Dwdm':dataPhid,'DefStr1':datacode};
