...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
    ...
    initComponent: function () {
		...
		var Type2=data[0].data.Type;
		var FThreshold;
		var righturl;
		if(Type2=='add'){
			FThreshold=0;
			righturl=C_ROOT + 'GQT/QT/ProjectThreshold/GetRightExpenseCategoryModel';
		}else{
			var data2=grid.getSelectionModel().getSelection();
			FThreshold=data2[0].data.FThreshold;
			righturl=C_ROOT + 'GQT/QT/ProjectThreshold/GetRightExpenseCategoryModel?Orgcode=' + datacode+'&PhId='+data2[0].data.PhId;
		}
        var me = this;
  ...
  },
  {
    ...
    "mapping": "Bz"
				...
				//url: C_ROOT + 'GQT/QT/ProjectThreshold/GetUnSetExpenseCategoryModel?Ocode=' + datacode,
				url: C_ROOT + 'GQT/QT/ProjectThreshold/GetLeftExpenseCategoryModel?Orgcode=' + datacode,
				autoLoad:false
			});
			storeZCLB.load();
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
            ...
            //url: C_ROOT + 'GQT/QT/ProjectThreshold/GetAlreadyExpenseCategoryModel?Ocode=' + datacode,
			url: righturl,
			autoLoad:false
            }),
        ...
        me.buttons = ['->', {
            ...
            iconCls: 'icon-Confirm',
            handler: function () {
				if (isNaN(Ext.getCmp('yuzhi').getValue())){
                ...
                var typeNames = "";
                var typeCodes = "";
                for (var i = 0; i < gridRight.store.data.items.length; i++) {
                    typeNames += gridRight.store.data.items[i].data.Mc + ",";
                    typeCodes += gridRight.store.data.items[i].data.Dm + ",";
                }
                if (typeNames.endsWith(",")) {
                    typeNames = typeNames.substring(0, typeNames.length - 1);
                }
                if (typeCodes.endsWith(",")) {
                    typeCodes = typeCodes.substring(0, typeCodes.length - 1);
                }
                /*var data = grid.getSelectionModel().getSelection();
                data[0].data.ProjTypeName = typeNames;
                data[0].data.ProjTypeId = typeCodes;
				data[0].data.FThreshold = parseFloat(Ext.getCmp('yuzhi').getValue()).toFixed(2);*/
				if(Type2=='add'){
					var rowdata = grid.getSelectionModel();
                    grid.getStore().insert(grid.getStore().getCount(), rowdata);
					grid.getStore().getAt(grid.getStore().getCount() - 1).set('ProjTypeName', typeNames);
					grid.getStore().getAt(grid.getStore().getCount() - 1).set('ProjTypeId', typeCodes);
					grid.getStore().getAt(grid.getStore().getCount() - 1).set('FThreshold', parseFloat(Ext.getCmp('yuzhi').getValue()).toFixed(2));
					grid.getStore().getAt(grid.getStore().getCount() - 1).set('Orgid', (gridOrg.getSelectionModel().getSelection())[0].data.PhId);
					grid.getStore().getAt(grid.getStore().getCount() - 1).set('Orgcode', (gridOrg.getSelectionModel().getSelection())[0].data.OCode);
