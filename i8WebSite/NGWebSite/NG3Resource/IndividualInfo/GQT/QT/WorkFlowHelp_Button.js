...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
  ...
  },
  {
    ...
    "mapping": "PhId"
            ...
            url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/GetIfNoButton?Dwdm=' + gridSet.getSelectionModel().getSelection()[0].data.PhId,
				autoLoad:false
			});
        var store = Ext.create('Ext.ng.JsonStore', {
			...
			url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/GetCorrespondenceSettingsListbyRelation'
        }),
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
            ...
            url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/GetIfButton?Dwdm=' + gridSet.getSelectionModel().getSelection()[0].data.PhId
		}),
        ...
        me.buttons = ['->', {
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					var record=resultStore.getAt(i);
                    var dataSet = gridSet.getSelectionModel().getSelection();
                    var SetPhid = dataSet[0].data.PhId;
                    var Setcode = dataSet[0].data.BZ;
					
					if(record.get('PhId')=="") {
						
                        InsertData.push({ 'PhId': record.get('zjphid'),'OCode':record.get('DM'),'key':record.get('zjphid')});
				...
				Ext.Ajax.request({
                    params: { 'Setcode': Setcode, 'SetPhId': SetPhid,'mydelete':mydelete,'myinsert':myinsert},
                    url: C_ROOT + 'GQT/QT/CorrespondenceSettings2/UpdateControlSet',
					success: function (response) {
						var resp = Ext.JSON.decode(response.responseText);
						if (resp.Status === "success"){
                            ...
                            Ext.Ajax.request({
                                params: { 'SetPhId': SetPhid },
                                url: C_ROOT + 'GQT/QT/QTControlSet/UpdateOrg',
                                success: function (response) {
                                    var storeSet = gridSet.getStore();
                                    storeSet.load();
                                }
                            });
						}else{
