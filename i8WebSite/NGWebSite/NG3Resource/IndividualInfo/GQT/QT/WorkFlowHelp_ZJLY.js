...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
  ...
  },
  {
    ...
    "mapping": "PhId"
				...
				url: C_ROOT + 'GQT/QT/SourceOfFunds/GetSourceOfFundsListNoDYGXdtl?Dwdm='+gridOrg.getSelectionModel().getSelection()[0].data.PhId,
				autoLoad:false
			});
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
			...
			autoLoad: false,
			//url: C_ROOT + 'GQT/QT/CorrespondenceSettings/GetCorrespondenceSettingsListbyRelation'
			url: C_ROOT + 'GQT/QT/SourceOfFunds/GetSourceOfFundsListDYGXdtl?Dwdm='+gridOrg.getSelectionModel().getSelection()[0].data.PhId
		}),
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=storeZJ.find('DM',Ext.getCmp('dingwei1').getValue());
						var mcindex=storeZJ.find('MC',Ext.getCmp('dingwei1').getValue());
						if(dmindex>=0){
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					...
					if(record.get('PhId')=="") {
						...
						InsertData.push({'PhId':record.get('zjphid'),'DM':record.get('DM'),'key':record.get('zjphid')});
				...
				Ext.Ajax.request({
						...
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings/UpdateZJLY',
						success: function (response) {
							var resp = Ext.JSON.decode(response.responseText);
							if (resp.Status === "success"){
								...
								var datacode=data[0].data.OCode;
								var dataPhid=data[0].data.PhId;
								var storeZJLY=gridZJLY.getStore();
