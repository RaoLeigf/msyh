...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
    ...
    initComponent: function () {
		...
		var storeZJ = Ext.create('Ext.ng.JsonStore', {
				autoLoad: false,
				//pageSize: 14,
				fields: [{
  ...
  },
  {
    ...
    "mapping": "OName"
            ...
            sorters: [{
				...
				url: C_ROOT + 'GQT/QT/VCorrespondenceSetting2/GetSpendUnRealdyList?dwdm='+gridOrg.getSelectionModel().getSelection()[0].data.OCode,
        ...
        var store = Ext.create('Ext.ng.JsonStore', {
			...
			//url: C_ROOT + 'GQT/QT/SourceOfFunds/GetSourceOfFundsList',
			url: C_ROOT + 'GQT/QT/CorrespondenceSettings/GetCorrespondenceSettingsListbyRelation',
        }),
		...
		gridLeft = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
		            ...
		            me.copyData(data, storeZJ,resultStore,'leftlabel','rightlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		gridRight = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
		            ...
		            //me.removeData(data,storeZJ, resultStore);
					me.copyData(data,resultStore,storeZJ,'rightlabel','leftlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		btnPanel = {
		    ...
		    items: [{
		        ...
		        handler: Ext.bind(function () {
		            var data = gridLeft.getSelectionModel().getSelection();
		            me.copyData(data, storeZJ,resultStore,'leftlabel','rightlabel');
					//me.moveData(data,storeZJ,resultStore);
		        })
		    }, {
		        xtype: 'button',
		        name: 'selectAll',
		        ...
		        handler: Ext.bind(function () {
		            ...
		            me.copyDataALL(storeZJ, resultStore,"leftlabel","rightlabel");
		        })
		    }, {
		        xtype: 'button',
		        name: 'removeSelect',
		        ...
		        handler: Ext.bind(function () {
					...
					me.copyDataALL(resultStore,storeZJ,"rightlabel","leftlabel");
		        })
		    }]
		},
		center = {
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
                        var dmindex = storeZJ.find('OCode',Ext.getCmp('dingwei1').getValue());
                        var mcindex = storeZJ.find('OName',Ext.getCmp('dingwei1').getValue());
						if(dmindex>=0){
						...
						//gridLeft.getSelectionModel().select(zjindex);
						
						//alert(Ext.getCmp('dingwei1').getValue());
					}
				}}
                }, {
				id:'leftlabel',
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
                        var dmindex = resultStore.find('DYDM',Ext.getCmp('dingwei2').getValue());
                        var mcindex = resultStore.find('Dymc',Ext.getCmp('dingwei2').getValue());
						if(dmindex>=0){
		...
		/*store.clearFilter();
		store.filter('Dwdm',dataPhid);
		store.filter('DefStr1',datacode);
		store.filter('Dylx','96');
		store.load(function(){
							for(var i=0;i<store.getCount();i++){
								var ZJLYDYDM=store.getAt(i).get('Dydm');
								var zjindex=storeZJ.find('DM',ZJLYDYDM);
								var ZJMC=storeZJ.getAt(zjindex).get('MC');
								//var ZJBZ=storeZJ.getAt(zjindex).get('BZ');
								store.getAt(i).set('MC',ZJMC);
								//store.getAt(i).set('BZ',ZJBZ);
							};
						});*/

										var params={'dwdm':datacode};
										Ext.apply(resultStore.proxy.extraParams,params);
										resultStore.load(function(){
											
											for(var i=0;i<resultStore.getCount();i++){
												var rec=resultStore.getAt(i);
												for(var j=0;j<storeZJ.getCount();j++){
													var sraw=storeZJ.getAt(j);
													if(sraw.data.OCode==rec.data.DYDM)
													{
														storeZJ.remove(sraw);
													}
												}
		...
		/*resultStore.clearFilter();				resultStore
		resultStore.filter('Dwdm',dataPhid);
		resultStore.filter('DefStr1',datacode);
		resultStore.filter('Dylx','96');
		resultStore.load(function(){
			for(var i=0;i<resultStore.getCount();i++){
								var ZJLYDYDM=resultStore.getAt(i).get('DM');
								var GXPhId=resultStore.getAt(i).get('PhId');
								var zjindex=storeZJ.find('DM',ZJLYDYDM);
								var ZJMC=storeZJ.getAt(zjindex).get('MC');
								resultStore.getAt(i).set('MC',ZJMC);
								DMData.push(ZJLYDYDM);
								PhIdData.push(GXPhId);
							};*/
							
							var rightStoreCount=resultStore.getCount();
							Ext.getCmp('rightlabel').setText("共"+rightStoreCount+"行");
							storeZJ.load(function(){
								for(var i=0;i<DMData.length;i++){
									var zjindex=storeZJ.find('DM',DMData[i]);
									storeZJ.removeAt(zjindex);
								};
								/*for(var i=0;i<storeZJ.getCount();i++){
									storeZJ.getAt(i).set('Dwdm',dataPhid);
									storeZJ.getAt(i).set('Dylx','96');
									storeZJ.getAt(i).set('DefStr1',datacode);
								};*/
								//gridLeft.getSelectionModel.selectRow(1);
								var leftStoreCount=storeZJ.getCount();
								Ext.getCmp('leftlabel').setText("共"+leftStoreCount+"行");
							});
		});
		
		//Ext.getCmp('leftlabel').setText('1');
        ...
        me.buttons = ['->', {
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					var rec=resultStore.getAt(i);
					selected+=rec.data.DYDM+",";
				}
				for(var i=0;i<storeZJ.getCount();i++){
					var rec=storeZJ.getAt(i);
					unselected+=rec.data.OCode+",";
				}
				Ext.Ajax.request({
					url: C_ROOT + 'GQT/QT/VCorrespondenceSetting2/UpdateR?dwdm=' + datacode+'&dwphid='+dataPhid+'&selected='+selected+'&unselected='+unselected,
					success: function (response)
					{
    ...
    copyData: function (selectData, fromStore,resultStore,fromlabel,resultlabel) {
        ...
        //var dataLen = selectData.length,
			...
			if(selectData[0].data.DWDM==null)
			{
				selectData[0].data.DYDM=selectData[0].data.OCode;
				selectData[0].data.Dymc=selectData[0].data.OName;
			}else{
				selectData[0].data.OCode=selectData[0].data.DYDM;
				selectData[0].data.OName=selectData[0].data.Dymc;
			}
			//tmpArr = me.currData.value,
			//tmpData = [];
        //for (var i = 0; i < dataLen; i++) {
            //var sourceData = selectData[i].data;
            //if (Ext.Array.indexOf(tmpArr, sourceData.DM) < 0) {
                //me.currData.value.push(sourceData.DM);
                //me.currData.text.push(sourceData.MC);
	...
	copyDataALL: function (fromStore,resultStore,fromlabel,resultlabel) {
		for(var i=0;i<fromStore.getCount();i++){
			...
			if(rec.data.DWDM==null)
			{
				rec.data.DYDM=rec.data.OCode;
				rec.data.Dymc=rec.data.OName;
			}else{
				rec.data.OCode=rec.data.DYDM;
				rec.data.OName=rec.data.Dymc;
			}
    ...
    /*removeData: function (data,fromStore, resultStore, isAll) {
        var me = this;
        if (isAll) {
			...
			var index = fromStore.getCount();
			fromStore.insert(index, data);
			resultStore.remove(data);
			
            for (var i = 0; i < data.length; i++) {
                var posIndex = Ext.Array.indexOf(data[i].data.DM);
