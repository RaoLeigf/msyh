...
Ext.define("Ext.ng.WFDynamicNodeUserWin", {
...
},
  {
    ...
    "mapping": "PhId"
				...
				url: C_ROOT + 'GQT/QT/BudgetAccounts/GetBudgetAccountsListNoDYGXdtl?Dwdm='+gridOrg.getSelectionModel().getSelection()[0].data.PhId,
				autoLoad:false
			});
		...
		resultStore = Ext.create('Ext.ng.JsonStore', {
			...
			autoLoad: false,
			url: C_ROOT + 'GQT/QT/BudgetAccounts/GetBudgetAccountsListDYGXdtl?Dwdm='+gridOrg.getSelectionModel().getSelection()[0].data.PhId
		}),
		...
		gridLeft = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
		            ...
		            me.copyData(data, storeYSKM,resultStore,'leftlabel','rightlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		gridRight = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
					...
					me.copyData(data,resultStore,storeYSKM,'rightlabel','leftlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		btnPanel = {
		    ...
		    items: [{
		        ...
		        handler: Ext.bind(function () {
		            ...
		            me.copyDataALL(storeYSKM, resultStore,"leftlabel","rightlabel");
		        })
		    }, {
		        xtype: 'button',
		        name: 'removeSelect',
		        ...
		        width: 60,
		        handler: Ext.bind(function () {
					me.copyDataALL(resultStore,storeYSKM,"rightlabel","leftlabel");
		        })
		    }]
		},
		center = {
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=storeYSKM.find('KMDM',Ext.getCmp('dingwei1').getValue());
						var mcindex=storeYSKM.find('KMMC',Ext.getCmp('dingwei1').getValue());
						if(dmindex>=0){
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=resultStore.find('KMDM',Ext.getCmp('dingwei2').getValue());
						var mcindex=resultStore.find('KMMC',Ext.getCmp('dingwei2').getValue());
						if(dmindex>=0){
		...
		storeYSKM.load(function(){
			Ext.getCmp('leftlabel').setText("共"+storeYSKM.getCount()+"行");
		});
		...
		/*var params={'Dylx':'02','Dwdm':dataPhid,'DefStr1':datacode};
										Ext.apply(resultStore.proxy.extraParams,params);
										resultStore.load(function(){
											storeYSKM.load();
											for(var i=0;i<resultStore.getCount();i++){
												var YSKMDM=resultStore.getAt(i).get('KMDM');
												var GXPhId=resultStore.getAt(i).get('PhId');
												//var YSKMindex=storeYSKM.find('KMDM',YSKMDM);
												var YSKMindex=me.findStoreIndex(storeYSKM,'KMDM',YSKMDM);
												if(YSKMindex>=0)
												{
													var YSKMMC=storeYSKM.getAt(YSKMindex).get('KMMC');
													resultStore.getAt(i).set('KMMC',YSKMMC);
													DMData.push(YSKMDM);
													PhIdData.push(GXPhId);
												}
											};
							...
							storeYSKM.load(function(){
								for(var i=0;i<DMData.length;i++){
									//var YSKMindex=storeYSKM.find('KMDM',DMData[i]);
									var YSKMindex=me.findStoreIndex(storeYSKM,'KMDM',DMData[i]);
									if(YSKMindex>=0)
									{
										storeYSKM.removeAt(YSKMindex);
									}
								};
								var leftStoreCount=storeYSKM.getCount();
            ...
            handler: function () {
				...
				for(var i=0;i<resultStore.getCount();i++){
					...
					if(record.get('PhId')=="") {
                        ...
                        InsertData.push({ 'PhId': record.get('yskmphid'), 'KMDM': record.get('KMDM'), 'key': record.get('yskmphid') });
				...
				Ext.Ajax.request({
						...
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings/UpdateYSKM',
						success: function (response) {
							var resp = Ext.JSON.decode(response.responseText);
							if (resp.Status === "success"){
								...
								var data = gridOrg.getSelectionModel().getSelection();
								var dataPhid=data[0].data.PhId;
								var storeYSKM=gridYSKM.getStore();
	...
	/*moveData: function (selectData, Store1,Store2) {
		var me = this;
    ...
    removeData: function (data,fromStore, resultStore, isAll) {
        ...
        if (isAll) {
            ...
            for (var i = 0; i < data.length; i++) {
                var posIndex = Ext.Array.indexOf(data[i].data.KMDM);
