...
//            me.dealOverFlowMenu();//解决用on方法绑定事件，折叠在menu中的按钮点击无效问题
//        });
//        //-------------判断toolbar多语言-------------
...
//        }
//    },
//    dealOverFlowMenu: function () {
//        var me = this;
//        var menuItems = me.layout.overflowHandler.menuItems;
//        if (menuItems) {
//            for (var i = 0; i < menuItems.length; i++) {
//                var itemId = menuItems[i].itemId;
//                var btn = me.get(itemId);
//                if (btn) {
//                    var handler = findClickFn(btn);
//                    //var hasClickListener = findClickFn(menuItems[i]);//解决按钮触发两次的问题，但是导致关闭按钮无效
//                    //if ((handler && !menuItems[i].handler) && !hasClickListener) {
//                    if (handler && !menuItems[i].handler) {
//                        menuItems[i].setHandler(handler);
//                        //menuItems[i].addListener('click', Test);
//                    }
//                }
//            }
//        }
        
//        function findClickFn(btn) {
//            if (Ext.isObject(btn.events.click)) {
//                return btn.events.click.listeners[0].fn;
//            }
//        }
//    }
//});
...
Ext.define('Ext.ng.Toolbar', {
    ...
    initComponent: function () {
        ...
        me.on("beforerender", this.beforeRender); //控制权限
        me.on('afterlayout', function () {
            me.dealOverFlowMenu();//解决用on方法绑定事件，折叠在menu中的按钮点击无效问题
        });
        ...
        var ToolbarLang;
        if (typeof ($ToolbarLang) !== "undefined") {
            ToolbarLanghtml = Ext.htmlDecode($ToolbarLang);
            ToolbarLang = Ext.decode(ToolbarLanghtml);
        } else {
            ToolbarLang = {};
        }
        this.ToolbarLang = ToolbarLang;
        //多语言的button
        this.createButton(); //创建按钮
    ...
    findButton: function (buttonid) {
        ...
        for (var i = 0; i < items.length; i++) {
            var btn = items[i];
            if ((buttonid === btn.itemId) || ((btn.itemId == undefined) && (buttonid === btn))) {
                if (buttonid in this.ToolbarLang) {
                    btn.text = this.ToolbarLang[buttonid]
                }
                return btn;
            }
        }
    ...
    findToorbarlang: function (langkey) {
        if (typeof ($ToolbarLang) !== "undefined") {
            var items = Object.keys(this.ToolbarLang);
            var itemslength = Object.keys(this.ToolbarLang).length;
            for (var i = 0; i < itemslength; i++) {
                //如果langkey和ToolbarLang的key相等，则返回ToolbarLang的value
                if (langkey == Object.keys(this.ToolbarLang)[i]) {
                    var toolbarkey = Object.keys(this.ToolbarLang)[i];
                    return this.ToolbarLang[toolbarkey];
                }
            }
        }
    ...
    setQueryInfo: function (pageid, callback) {
        ...
        function Save() {
            ...
            Ext.Ajax.request({
                ...
                params: {
                    'griddata': griddata, 'gridchange': gridchange
                },
                callback: callback,
    ...
    initComponent: function () {
        ...
        combo = Ext.create('Ext.form.ComboBox', {
            ...
            store: new Ext.data.ArrayStore({
                fields: ['text', 'value'],
                data: [['20', 20], ['25', 25], ['30', 30], ['40', 40], ['50', 50], ['100', 100], ['150', 150], ['200', 200]]
            }),
            valueField: 'value',
            displayField: 'text',
