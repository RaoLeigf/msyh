...
using System.Linq;
using System.Text;
//using GCW3.CW.Common.ApiMdel.BaseModel;
...
namespace GData3.Common.Utils
{
    public class DCHelper
    {
        ...
        public static void ConvertDic<T>(Dictionary<string, object> dic) where T : class
        {
            ...
            if (!dic.Any(x => x.Key.Contains("*dictionary*or"))) return;  //貌似只有使用or才会出错，所以没有or时就不出来了
            ...
            foreach (string key in keys)
            {
                ...
                if (key.Contains("[or-dictionary0") || key.Contains("]*dictionary*or"))
                {
                    var value = dic[key];
                    if (value is JObject)
                    {
