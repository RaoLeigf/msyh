  ...
  },
  {
    ...
    "mapping": "MC"
		...
		gridLeft = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
		            ...
		            me.copyData(data, storeYSK,resultStore,'leftlabel','rightlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		gridRight = Ext.create('Ext.ng.GridPanel', {
		    ...
		    listeners: {
		        'itemdblclick': function (item, record, it, index, e, eOpts) {
					...
					me.copyData(data,resultStore,storeYSK,'rightlabel','leftlabel');
		        }
		    },
		    viewConfig: {
		        style: {
		...
		btnPanel = {
		    ...
		    items: [{
		        ...
		        handler: Ext.bind(function () {
		            ...
		            me.copyDataALL(storeYSK, resultStore,"leftlabel","rightlabel");
		        })
		    }, {
		        xtype: 'button',
		        name: 'removeSelect',
		        ...
		        width: 60,
		        handler: Ext.bind(function () {
					me.copyDataALL(resultStore,storeYSK,"rightlabel","leftlabel");
		        })
		    }]
		},
		center = {
		    ...
		    items: [{
				...
				listeners:{specialkey:function(textfield,e){
					if(e.getKey()==Ext.EventObject.ENTER){
						var dmindex=storeYSK.find('DM',Ext.getCmp('dingwei1').getValue());
						var mcindex=storeYSK.find('MC',Ext.getCmp('dingwei1').getValue());
						if(dmindex>=0){
        ...
        me.buttons = ['->', {
            ...
            iconCls: 'icon-Confirm',
            handler: function () {
				//var mydata = {mstformData: "{"form":{"key":"PhId","newRow":{"DYDM":"100","DYLX":"96","PhId":"","key":""}}}"};

				/*var ZJLYlist=new Array();
				for(var i=0;i<resultStore.getCount();i++){
					var record=resultStore.getAt(i);
					ZJLYlist.push(record.get('DM'));
				}*/		
				//alert(ZJLYlist);
				//alert(PhIdData.length);
				
				for(var i=0;i<PhIdData.length;i++){
					Ext.Ajax.request({
						params:{phid:PhIdData[i]},
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings/DeletebyId',
						success: function (response) {
				...
				for(var i=0;i<resultStore.getCount();i++){
					...
					//var data2='{"form":{"key":"PhId","newRow":{"Dwdm":,"Dydm":"'+record.get('DM')+'","Dylx":"96","PhId":"","NgRecordVer":"","key":""}}}';
					var data2='{"form":{"key":"PhId","newRow":{"Dwdm":"'+orgPhid+'","Dydm":"'+record.get('DM')+'","Dylx":"03","DefStr1":"'+orgcode+'","PhId":"","NgRecordVer":"","key":""}}}';
					Ext.Ajax.request({
						params:{data:data2},
						url: C_ROOT + 'GQT/QT/CorrespondenceSettings/SaveZJLY',
						success: function (response) {
							var resp = Ext.JSON.decode(response.responseText);
